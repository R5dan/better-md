...
class BaseExtension(Extension):
    ...
    def end_list(self):     
        def data2li(ul: 'UL_LIST|OL_LIST') -> 'LIST_ITEM':
            return {
                "data": ul,
                "dataType": "item"
            }
        
        def new_list(parent:'LIST_TYPE', list:'LIST_TYPE'):
            if parent is None:
                lists.append(list)
                return
            
            elif list[] > parent["indent"]:
        ...
        for item in self.list_stack:
            if new:
                new = False
                cur_indent = item["indent"]
                cur_list = {
                    "value": [],
                    "parent": None,
                    "type": list_modes[item["type"]],
                    "key": item["type"],
                    **({"start": item["num"]} if item["list"] == "ol" else {}),
                    "dataType": "list"
                }
                lists.append(cur_list)
            
            elif cur_indent > item["indent"]:
        ...
        def handle_child_list(child_list:'LIST_TYPE'):
            return self.create_element(
                ...
                [
                    handle_child_list(child) if (lambda child: print(child) or (child["dataType"] == "list"))(child) else handle_item(child) for child in child_list["value"]
                ]
            )
